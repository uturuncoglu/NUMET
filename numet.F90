!==============================================================================
! NUMET (Nuopc Unified Modeling Environment Toolkit)
...
program numetApp
  ...
  use ESMF
  use NUOPC
  use numetDriver, only: driverSS => SetServices
  ...
  ! Initialize ESMF
  call ESMF_Initialize(configFileName="nuRun.config", config=config, &
  ...
  call ESMF_LogWrite("numetApp STARTING", ESMF_LOGMSG_INFO, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
    line=__LINE__, &
    file=__FILE__)) &
    call ESMF_Finalize(endflag=ESMF_END_ABORT)
  ...
  ! Create the numet driver
  driver = ESMF_GridCompCreate(name="numetDriver", config=config, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
    line=__LINE__, &
    file=__FILE__)) &
    call ESMF_Finalize(endflag=ESMF_END_ABORT)

  ! SetServices for the numet driver
  ...
  ! Call Initialize for the numet driver
  ...
  ! Call Finalize for the numet driver
  ...
  call ESMF_LogWrite("numetApp FINISHED", ESMF_LOGMSG_INFO, rc=rc)
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
    line=__LINE__, &
    file=__FILE__)) &
    call ESMF_Finalize(endflag=ESMF_END_ABORT)
